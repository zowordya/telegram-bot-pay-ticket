...
async def ticket_standard_callback(callback: CallbackQuery):
    ...
    await callback.message.answer_photo(
        ...
        caption="📝 ***Вы выбрали Танцпол билет***\n"
                ...
                "Если возникнут вопросы, обращайтесь к организаторам через наш чат поддержки @shitstillgoesbrazy.",
        parse_mode="Markdown",
...
async def handle_pdf_upload(message: Message):
    ...
    if result_line_sum.strip() == "690,00 ₽" and result_line_name.strip() == "Тимур Кириллович П.":
    ...
    elif result_line_sum.strip() == "849,00 ₽" and result_line_name.strip() == "Тимур Кириллович П.":
        database.update_payment_status(user_id, "Оплачено")
    ...
    elif result_line_sum.strip() == "1390,00 ₽" and result_line_name.strip() == "Тимур Кириллович П.":
...
async def help_user(callback: CallbackQuery):
    ...
    await callback.message.answer(
        text='Если у вас возникли проблемы с ботом.\n'
             'Напишите нашему администратору @shitstillgoesbrazy.',
        reply_markup=kb.cancalls
    )
